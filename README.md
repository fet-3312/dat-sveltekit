# DTA SvelteKit 開發樣板

DTA 團隊的 SvelteKit 專案樣板，提供標準化的開發環境與最佳實踐配置。

---

## 📦 團隊成員如何使用此樣板

### 前置需求

確保已安裝以下工具：

```sh
# 檢查 Node.js 版本（需 >= 20.x）
node -v

# 檢查 pnpm 版本（需 >= 9.x）
pnpm -v

# 如果尚未啟用 pnpm，請執行
corepack enable pnpm
corepack prepare pnpm@latest --activate
```

### 建立新專案（3 步驟）

```sh
# 1️⃣ 複製樣板到新目錄
npx degit fet-3312/dat-sveltekit my-project

# 2️⃣ 進入目錄並執行初始化
cd my-project
node setup.js

# 3️⃣ 安裝依賴並啟動
pnpm install
pnpm dev
```

### 初始化過程說明

執行 `node setup.js` 時，腳本會互動式詢問：

```
🏢 DTA SvelteKit 專案初始化

? 專案名稱 (my-project): my-awesome-app
? 專案描述: 我的超棒應用程式

✔ 已更新 package.json
✔ 已更新 README.md
✔ 已建立 .env
✔ 已初始化 Git
✔ 已清理樣板檔案

✔ 專案初始化完成！

  下一步：
  pnpm install
  pnpm dev
```

**腳本會自動完成：**

| 動作                | 說明                                   |
| ------------------- | -------------------------------------- |
| 更新 `package.json` | 設定專案名稱、描述，版本重設為 `0.0.1` |
| 更新 `README.md`    | 替換標題為你的專案名稱                 |
| 建立 `.env`         | 從 `.env.example` 複製環境變數範本     |
| 初始化 Git          | 執行 `git init`（如果需要）            |
| 清理樣板檔案        | 刪除 `setup.js` 等只有樣板需要的檔案   |

### 一行指令快速建立

如果你熟悉流程，可以用一行指令完成：

```sh
npx degit fet-3312/dat-sveltekit my-project && cd my-project && node setup.js
```

然後依照提示輸入專案名稱即可。

---

## 專案簡介

這是一個基於 **SvelteKit 2 + Svelte 5** 的靜態網站開發樣板，專為 DTA 團隊設計。

### 主要特色

- ⚡ **Svelte 5** — 使用最新的 Runes 響應式語法
- 🏗️ **SvelteKit 2** — 檔案式路由與現代化建構
- 📦 **靜態部署** — 透過 `adapter-static` 輸出純靜態檔案
- 🎨 **PostCSS** — 支援巢狀語法與現代 CSS 特性
- ✅ **TypeScript** — 完整的型別安全
- 🧪 **測試整合** — Vitest (單元) + Playwright (E2E)
- 🔧 **程式碼品質** — ESLint + Prettier + Husky 預提交檢查

---

## 系統需求

| 工具    | 版本需求 |
| ------- | -------- |
| Node.js | >= 20.x  |
| pnpm    | >= 9.x   |

> ⚠️ 本專案**僅支援 pnpm**，請勿使用 npm 或 yarn。

---

## 🚀 框架版本

本專案始終保持在**最新的 SvelteKit 和 Svelte 穩定版本**。

**最後更新**：2026-02-09

| 套件       | 版本   | 狀態    |
| ---------- | ------ | ------- |
| SvelteKit  | 2.50.2 | ✅ 最新 |
| Svelte     | 5.50.0 | ✅ 最新 |
| Vite       | 7.3.1  | ✅ 最新 |
| TypeScript | 5.9.3  | ✅ 最新 |
| ESLint     | 10.0.0 | ✅ 最新 |
| Prettier   | 3.8.1  | ✅ 最新 |

完整的依賴版本清單見 [.github/VERSIONS.md](.github/VERSIONS.md)。

### 升級與維護

- 📖 **升級流程** — [.github/TEMPLATE_UPGRADE_PROCESS.md](.github/TEMPLATE_UPGRADE_PROCESS.md)
- ✅ **升級檢查清單** — [.github/UPGRADE_CHECKLIST.md](.github/UPGRADE_CHECKLIST.md)
- 📋 **版本歷史** — [.github/VERSIONS.md](.github/VERSIONS.md)

每次新項目通過 `setup.js` 初始化時，都會自動使用此範本中的最新版本。

---

## 可用指令

| 指令               | 說明                               |
| ------------------ | ---------------------------------- |
| `pnpm dev`         | 啟動開發伺服器                     |
| `pnpm build`       | 建構正式版本                       |
| `pnpm preview`     | 預覽正式版本                       |
| `pnpm check`       | 執行 Svelte 與 TypeScript 型別檢查 |
| `pnpm check:watch` | 監聽模式型別檢查                   |
| `pnpm lint`        | 執行 ESLint 與 Prettier 檢查       |
| `pnpm format`      | 自動格式化程式碼                   |
| `pnpm test`        | 執行所有測試                       |
| `pnpm test:unit`   | 執行單元測試 (Vitest)              |
| `pnpm test:e2e`    | 執行端對端測試 (Playwright)        |

---

## 專案結構

```
dta-sveltekit/
├── src/
│   ├── routes/          # SvelteKit 頁面路由
│   │   ├── +layout.svelte
│   │   └── +page.svelte
│   ├── lib/             # 共用元件與工具
│   │   ├── assets/      # 圖片、字型等靜態資源
│   │   └── index.ts     # 模組匯出入口
│   ├── app.html         # HTML 模板
│   └── app.d.ts         # 全域型別定義
├── static/              # 不經處理的靜態檔案
├── e2e/                 # Playwright E2E 測試
├── svelte.config.js     # SvelteKit 配置
├── vite.config.ts       # Vite 配置 (含 Vitest)
├── tsconfig.json        # TypeScript 配置
├── eslint.config.js     # ESLint 配置
└── playwright.config.ts # Playwright 配置
```

---

## 環境變數

本專案使用 SvelteKit 的環境變數機制。公開變數需以 `PUBLIC_` 前綴命名。

| 變數名稱           | 說明                                | 必填 | 預設值 | 範例      |
| ------------------ | ----------------------------------- | :--: | ------ | --------- |
| `PUBLIC_BASE_PATH` | 應用程式的子路徑 (用於非根目錄部署) |  否  | `""`   | `/my-app` |

### 設定方式

1. 複製範本檔（如有提供）：

   ```sh
   cp .env.example .env
   ```

2. 或直接建立 `.env` 檔案：
   ```env
   PUBLIC_BASE_PATH=/my-app
   ```

> 💡 靜態部署時，執行階段配置建議透過 `static/` 目錄下的 JSON 檔案載入。

---

## 開發規範

### Svelte 5 Runes

本專案使用 Svelte 5 的 Runes 語法，請使用 `$state`、`$derived`、`$effect` 等新語法，避免使用舊版 `onMount` / stores 模式。

```svelte
<script lang="ts">
	let count = $state(0)
	let doubled = $derived(count * 2)
</script>
```

### 樣式撰寫

使用 PostCSS 巢狀語法：

```svelte
<style lang="postcss">
	.container {
		padding: 1rem;

		& .title {
			font-size: 1.5rem;
		}
	}
</style>
```

### 路徑處理

由於可能部署至子路徑，請使用 `$app/paths` 處理連結：

```svelte
<script>
	import { base } from '$app/paths'
</script>

<a href="{base}/about">關於我們</a>
<img src="{base}/images/logo.png" alt="Logo" />
```

---

## 測試

### 單元測試 (Vitest)

```sh
pnpm test:unit
```

測試檔案命名規則：

- Svelte 元件測試：`*.svelte.spec.ts`
- 一般單元測試：`*.spec.ts` 或 `*.test.ts`

### E2E 測試 (Playwright)

```sh
pnpm test:e2e
```

測試檔案位於 `e2e/` 目錄。

---

## 建構與部署

### 建構正式版本

```sh
pnpm build
```

建構產物輸出至 `build/` 目錄，可直接部署至任何靜態檔案伺服器。

### 本機預覽

```sh
pnpm preview
```

預覽伺服器運行於 <http://localhost:4173>

### 部署注意事項

- ✅ 本專案使用 `adapter-static`，僅產出靜態檔案
- ✅ 使用 `PUBLIC_BASE_PATH` 環境變數支援子路徑部署
- ❌ 不支援 Server-Side Rendering (SSR) 或 API Routes
- 🌐 支援部署至 GitHub Pages、Netlify、Vercel (靜態模式) 等平台

---

## 程式碼品質

提交前請確保：

```sh
pnpm format   # 格式化程式碼
pnpm check    # 型別檢查
pnpm lint     # Lint 檢查
```

Husky 會在 `git commit` 時自動執行預提交檢查。

---

## 貢獻指南

1. 遵循 [Copilot 指引](.github/copilot-instructions.md) 中的開發規範
2. 確保所有檢查通過後再提交
3. 撰寫清楚的 commit message

---

## 📚 文檔與資源

### 項目文檔

- [Copilot 編碼指南](.github/copilot-instructions.md) — 開發規範和最佳實踐
- **🤖 [升級自動化](./github/agents/sveltekit-project-upgrade.agent.md)** — **新：Copilot Agent 自動升級任何 SvelteKit 項目**
- [框架升級流程](.github/TEMPLATE_UPGRADE_PROCESS.md) — 如何升級至最新框架（手動流程）
- [升級檢查清單](.github/UPGRADE_CHECKLIST.md) — 升級時的驗證步驟
- [版本歷史](.github/VERSIONS.md) — 框架版本更新記錄

### Copilot Agents

此項目提供以下 Copilot Agents 協助開發：

| Agent | 用途 |
|-------|------|
| [sveltekit-project-upgrade.agent.md](.github/agents/sveltekit-project-upgrade.agent.md) | ⭐ **推薦**：自動升級任何 SvelteKit 項目至最新官方框架版本 |
| [svektekit-update.agent.md](.github/agents/svektekit-update.agent.md) | 升級既有專案至最新 Svelte 5 / SvelteKit 2 基準 |

### 官方資源

- [SvelteKit 官方文檔](https://svelte.dev/docs/kit)
- [Svelte 5 Runes 指南](https://svelte.dev/docs/svelte/runes)
- [Vite 官方文檔](https://vitejs.dev/)
- [TypeScript 文檔](https://www.typescriptlang.org/docs/)
- [ESLint 官方文檔](https://eslint.org/)

---

## 授權

DTA 團隊內部使用
